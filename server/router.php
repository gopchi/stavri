...
if($request->get('authors')) {
	$response->authors = $db->querybind_all('SELECT authors.id, A_name, age, name FROM authors INNER JOIN countries ON authors.country_id=countries.id');

}
// else if($request->get('authors/[0-9]+')) {
// 	$authorID = (int) $request->segment(1);
// 	$response->person = $db->querybind_one('SELECT * FROM authors WHERE id = ?', [ $authorID ]);
// 	if(!$response->person) {
// 		$response->code(404);
